...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      ...
      { title: "Vestui — Gestão inteligente & Loja online para moda" },
      {
        name: "description",
        content:
...
function RootComponent() {
  ...
  useEffect(() => {
    const { data } = supabase.auth.onAuthStateChange((event) => {
      const routeInvalidatingEvents = ["SIGNED_IN", "SIGNED_OUT", "USER_UPDATED"] as const;
      if (!(routeInvalidatingEvents as readonly string[]).includes(event)) return;
      router.invalidate();
