...
function VisaoGeral() {
  ...
  const togglePrivacidade = () => {
    ...
    toast.info(nextVal ? "Modo Balcão Ativado" : "Modo Balcão Desativado", {
      description: nextVal ? "Valores monetários ocultados para proteção na loja física." : "Valores visíveis.",
    });
  };
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground leading-relaxed flex flex-wrap items-center gap-1.5">
            <span>Sua vitrine está no ar em</span>
            <span className="inline-flex items-center rounded-md bg-secondary/80 px-2 py-0.5 font-mono text-[11px] font-medium text-foreground border border-border/50">
              {vitrineDisplay}
            </span>
            <span className="text-muted-foreground/60 hidden sm:inline">·</span>
            <span className="text-muted-foreground/80 hidden sm:inline">Sincronização automática com o estoque.</span>
          </p>
        </div>
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2">
          ...
          >
            ...
            <span className="hidden sm:inline">
              {ocultarSaldos ? "Modo Balcão Ativo" : "Ocultar Saldos"}
            </span>
          </button>

          {/* Secundário: Copiar Link */}
          <button
            type="button"
            onClick={copiarLink}
